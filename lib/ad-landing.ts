...
export function formatAdArrivalDirective(match: AdOpenerMatch): string {
  if (match.language === 'en') {
    return [
      ...
      '• Keep it to ONE warm discovery question — do NOT list specs (Wh / W / voltage) or recharge details; you have not learned their load yet, so numbers would just be a spec dump.',
      '• Save all runtime/duration numbers for AFTER they tell you what they run — never promise hours or "almost 2 days" on turn 1.',
  ...
  return [
    ...
    '• Quédate en UNA pregunta de descubrimiento cálida — NO listes specs (Wh / W / voltaje) ni detalles de recarga; aún no conoces su carga, así que los números serían solo un volcado.',
    '• Guarda todos los números de duración/horas para DESPUÉS de que diga qué conecta — nunca prometas horas ni "casi 2 días" en el turno 1.',
