...
- 2026-07-13 — Sorteo answers for Sol: knowledge_base entry (live immediately, no deploy) with the full participation steps (follow FB+IG @oiikon, comment "QUIERO", register at oiikon.com/sorteo), prize (E2000LFP ~$599), eligibility (US 18+, ends 7/19) and the link-on-its-own-line rule so WhatsApp makes it clickeable; AGENT_PROMPT.md giveaway block aligned (added the Instagram step + link-sharing rule).
- 2026-07-11 — ECO-WORTHY all-in-one systems live in Sol: the 3 new store products (EW-AIO-3.5K $1,299.99 · EW-AIO-5K $1,499.99 · EW-AIO-10K $4,799.99 split-phase 240V) added to agent_product_catalog with full specs, ideal-for and expansion/panel compatibility notes, and AGENT_PROMPT.md now makes them the DEFAULT Level-3 answer (quotable at catalog price even >$3,000 — single SKUs, not custom combos; electrician warning still mandatory; 10-yr battery warranty as selling point).
- 2026-07-10 — PowerPal Y4 14.6kWh price audit: the store showed $775 instead of $1,990 — a 61.0553% discount saved during the 7/8 product edit preserved the retired 10kWh model's $775 price on the new 14.6kWh product (cost $674 → margin was ~$101). Fixed: price $1,990, discount 0, on the store AND Sol's catalog (Sol's row still carried the dead PP-Y4-10KWH SKU/price — renamed to PP-Y4-14.6KWH, in stock ×2, so the 12-day 10-minute sync ping-pong on that orphan SKU ends too). Catalog-wide sweep: no other product has a stale-price discount; the other high-percentage discounts are the deliberate MSRP-anchor model.
- 2026-07-09 — PECRON supply update (Yuki 7/9): E1500LFP can't ship → pre-order sales stopped (station + Kit Hogar 1500 now out_of_stock); E2000LFP production ended → marked `discontinued` and Sol now says so honestly (no back-in-stock promises, steers to E2400/E3600/F3000); E2400LFP batch (700 units, 200 reserved, LA port 7/14) → pre-order with published ETA — Sol/store show "disponible alrededor del 19 de julio" (in stock & deliverable 7/19, Ed confirmed).
- 2026-07-09 — Bundle→base stock rule: kits showing available while their power station was out (Kit Hogar 2400 → E2400LFP, Kit Hogar 5000 → F5000LFP) fixed in both the store and Sol's catalog, and the 10-min sync cron now enforces it automatically — a kit flips out of stock (inheriting the base's status) the moment its station does, on both sides. Base derived from the kit's name so new kits are covered; expansion batteries deliberately exempt.
